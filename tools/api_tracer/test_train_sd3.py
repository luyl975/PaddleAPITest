...
def main(args):
    ...
    transformer = SD3Transformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path,
            device_map="auto", subfolder="transformer", revision=args.revision, variant=args.variant
    )
    ...
    def load_model_hook(models, input_dir):
        for _ in range(len(models)):
            ...
            if isinstance(unwrap_model(model), SD3Transformer2DModel):
                load_model = SD3Transformer2DModel.from_pretrained(input_dir, torch_dtype=torch.bfloat16,
                    device_map="auto", subfolder="transformer")
                model.register_to_config(**load_model.config)

                model.load_state_dict(load_model.state_dict())
            elif isinstance(unwrap_model(model), (CLIPTextModelWithProjection, T5EncoderModel)):
                try:
                    load_model = CLIPTextModelWithProjection.from_pretrained(input_dir, torch_dtype=torch.bfloat16,
                        device_map="auto", subfolder="text_encoder")
                    model(**load_model.config)
                    model.load_state_dict(load_model.state_dict())
                except Exception:
                    try:
                        load_model = CLIPTextModelWithProjection.from_pretrained(input_dir, torch_dtype=torch.bfloat16,
                            device_map="auto", subfolder="text_encoder_2")
                        model(**load_model.config)
                        model.load_state_dict(load_model.state_dict())
                    except Exception:
                        try:
    ...
    if accelerator.is_main_process:
        ...
        if args.train_text_encoder:
            ...
            pipeline = StableDiffusion3Pipeline.from_pretrained(
                ...
                device_map="balanced", transformer=transformer
            )
...
if __name__ == "__main__":
    args = parse_args()
    output_path = f"/root/paddlejob/PaddleAPITest/tools/api_tracer/trace_output_test_train/stabilityai/stable-diffusion-3-medium-diffusers"
    tracer = APITracer(
        "torch",
    ...
    try:
        ...
        traceback.print_exc()
        print(f"❌ An error occurred during inference for stabilityai/stable-diffusion-3-medium-diffusers : {e}")
    finally:
        tracer.stop()
