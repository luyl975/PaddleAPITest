...
def main(args):
    ...
    transformer = FluxTransformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path, device_map="balanced", subfolder="transformer", revision=args.revision, variant=args.variant
    )
    ...
    def load_model_hook(models, input_dir):
        for _ in range(len(models)):
            ...
            if isinstance(unwrap_model(model), FluxTransformer2DModel):
                load_model = FluxTransformer2DModel.from_pretrained(input_dir, device_map="balanced", subfolder="transformer")
    ...
    if accelerator.is_main_process:
        ...
        if args.train_text_encoder:
            ...
            pipeline = FluxPipeline.from_pretrained(args.pretrained_model_name_or_path, device_map="balanced", transformer=transformer)
...
if __name__ == "__main__":
    args = parse_args()
    output_path = f"/root/paddlejob/PaddleAPITest/tools/api_tracer/trace_output_test_train/black-forest-labs/FLUX.1-dev"
    tracer = APITracer(
        "torch",
    ...
    try:
        ...
        traceback.print_exc()
        print(f"❌ An error occurred during inference for black-forest-labs/FLUX.1-dev: {e}")
    finally:
        tracer.stop()
